...
function recordMatchProfile(win) {
  ...
  pushProfileActivity({
    ...
    copy: `${getTeamDisplayName(currentTeam)} · ${win ? `рахунок ${myScore}:${oppScore}` : `рахунок ${myScore}:${oppScore}`}`,
    accent: win ? 'win' : 'loss',
  });
...
function recordDreamProfile() {
  const previousUnlocked = getUnlockedAchievementIds();
  const team = getDreamTeam();
  const average = getDreamAverageRating();
...
function recordGoatProfile(winner) {
  if (!winner) return;
  const previousUnlocked = getUnlockedAchievementIds();
...
function getAchievementProgress(achievement, quizAccuracy, winRate) {
  switch (achievement.id) {
    case 'first-match':
      return {
        current: Math.min(profileState.matchesPlayed, 1),
        target: 1,
        label: `${profileState.matchesPlayed}/1 матч`,
      };
    case 'streak-3':
      return {
        current: Math.min(profileState.bestWinStreak, 3),
        target: 3,
        label: `${profileState.bestWinStreak}/3 win streak`,
      };
    case 'quiz-master':
      return {
        current: Math.min(quizAccuracy, 80),
        target: 80,
        label: `${quizAccuracy}% / 80% accuracy`,
      };
    case 'goat-crown':
      return {
        current: Math.min(profileState.goatRuns, 1),
        target: 1,
        label: `${profileState.goatRuns}/1 GOAT run`,
      };
    case 'dream-architect':
      return {
        current: Math.min(profileState.dreamBuilds, 1),
        target: 1,
        label: `${profileState.dreamBuilds}/1 Dream XI`,
      };
    case 'centurion': {
    ...
    case 'veteran': {
      ...
      return {
        current: Math.min(totalRuns, 10),
        target: 10,
        label: `${totalRuns}/10 сесій`,
      };
    }
    case 'ice-cold':
      return {
